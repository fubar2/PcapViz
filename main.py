...
parser = ArgumentParser(description='pcap topology and message mapper. Draws all 3 layers unless a single one is specified')
parser.add_argument('-i', '--pcaps', nargs='*',help='space delimited list of capture files to be analyzed',required=True)
...
def doPcap(pin,args):
	...
	if not (args.layer2 or args.layer3 or args.layer4): # none requested - do all
		for layer in [2,3,4]:
			doLayer(layer, packets,fname,args)
	else:
		layer = 3
		if args.layer2:
			layer = 2
